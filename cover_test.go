...
// smap returns a map[Subset]struct{} populated with ss.
func smap(ss ...Subset) map[Subset]struct{} {
	m := make(map[Subset]struct{}, len(ss))
	for _, s := range ss {
		m[s] = struct{}{}
	}
	return m
...
// emap returns a map[Element]struct{} populated with es.
func emap(es ...Element) map[Element]struct{} {
	m := make(map[Element]struct{}, len(es))
	for _, e := range es {
		m[e] = struct{}{}
	}
	return m
...
func TestAdd(t *testing.T) {
	for _, test := range []struct {
		...
		// Subset containing one Element
		{
			...
			&Cover{
				inss: map[Subset]map[Element]struct{}{true: emap(true)},
				ines: map[Element]map[Subset]struct{}{true: smap(true)},
...
func TestDominates(t *testing.T) {
	for _, test := range []struct {
		c   *Cover
		dom map[Subset]map[Subset]struct{}
	}{
		{
			fromInputs(input{true, []Element{true}}),
			map[Subset]map[Subset]struct{}{},
		},
		{
			fromInputs(
				input{"A", []Element{"x"}},
		...
		},
		{
			...
			map[Subset]map[Subset]struct{}{"B": smap("A")},
		},
		{
			fromInputs(
				input{"A", []Element{2}},
...
func allMatch(a, b [][]Subset) bool {
	bms := make([]map[Subset]struct{}, len(b))
	for i, bs := range b {
		bms[i] = smap(bs...)
	}
	for _, as := range a {
...
var coverTests = map[string]struct {
	...
	"tautology": {
		...
		s: &Cover{
			inss: map[Subset]map[Element]struct{}{true: emap(true)},
			ines: map[Element]map[Subset]struct{}{true: smap(true)},
			ss:   map[Subset]map[Element]struct{}{true: emap(true)},
			es:   map[Element]map[Subset]struct{}{true: smap(true)},
	...
	"disjoint A and B": {
		...
		s: &Cover{
			inss: map[Subset]map[Element]struct{}{"A": emap("x"), "B": emap("y")},
			ines: map[Element]map[Subset]struct{}{"x": smap("A"), "y": smap("B")},
			ss:   map[Subset]map[Element]struct{}{"A": emap("x"), "B": emap("y")},
			es:   map[Element]map[Subset]struct{}{"x": smap("A"), "y": smap("B")},
	...
	"B contains A": {
		...
		s: &Cover{
			inss: map[Subset]map[Element]struct{}{"A": emap("x"), "B": emap("x", "y", "z")},
			ines: map[Element]map[Subset]struct{}{"x": smap("A", "B"), "y": smap("B"), "z": smap("B")},
			ss:   map[Subset]map[Element]struct{}{"B": emap("x", "y", "z")},
			es:   map[Element]map[Subset]struct{}{"x": smap("B"), "y": smap("B"), "z": smap("B")},
		...
		e: &Cover{
			inss: map[Subset]map[Element]struct{}{"A": emap("x"), "B": emap("x", "y", "z")},
			ines: map[Element]map[Subset]struct{}{"x": smap("A", "B"), "y": smap("B"), "z": smap("B")},
			ss:   map[Subset]map[Element]struct{}{"A": emap()},
			es:   map[Element]map[Subset]struct{}{},
	...
	"seven-segment A": {
		...
		e: &Cover{
			...
			ss: map[Subset]map[Element]struct{}{"--10": emap(), "1--0": emap(12), "11-0": emap(12)},
			es: map[Element]map[Subset]struct{}{12: smap("1--0", "11-0")},
		...
		sim: &Cover{
			...
			ss: map[Subset]map[Element]struct{}{"--10": emap()},
			es: map[Element]map[Subset]struct{}{},
	...
	"seven-segment D": {
		...
		sim: &Cover{
			...
			ss: map[Subset]map[Element]struct{}{"001-": emap()},
			es: map[Element]map[Subset]struct{}{},
